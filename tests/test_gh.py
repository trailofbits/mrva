...
async def test_submit_variant_analysis():
    ...
    async with gh.Client(token="fake_token", base_url="https://api.github.com", timeout=None) as client:
        client.client.post = unittest.mock.AsyncMock(return_value=success_response)
        payload = {"action_repo_ref": "main", "language": "python", "query_pack": "base64data", "repository_lists": ["top_100"]}
...
async def test_get_variant_analysis_repo_task():
    success_response = httpx.Response(200, json={"repository": {"id": 99, "full_name": "owner/repo"}, "analysis_status": "succeeded", "artifact_url": "https://example.com/artifact.zip"})
